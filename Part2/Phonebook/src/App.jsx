...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if (existingPerson) {
      if (
        window.confirm(
          `${newName} has already been added to the notebook, replace the old number with the new one?`,
        )
      ) {
        personsService
  ...
  return (
    <div>
      <h2>Phonebook</h2>
      <Notification
        message={notification === null ? null : notification.message}
        isError={notification === null ? null : notification.isError}
      />
